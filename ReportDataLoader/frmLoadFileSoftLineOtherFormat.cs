...
namespace WinTest
{
    public partial class frmLoadFileSoftLineOtherFormat : Form
    {
        DataSet LoadDataSet;
        ...
        public frmLoadFileSoftLineOtherFormat()
        {
        ...
        private void SaveContacts(DataTable t, BackgroundWorker bw)
        {
            ...
            using (GetData data = new GetData())
            {
                if (this.CampaignId > 0)
                {
                    ...
                    {
                        string phone = row["Номер телефона"].ToString();
                        string status = row["статус SMS"].ToString();
                        string chanel = row["Мобильный оператор"].ToString();
